...
# 加载数据
data = pd.read_csv('data1/01-vertebral.csv')
...
for i in range(len(test_positive_samples)):
    ...
    revise_params = {
        "data_name": "vertebral",
        "lambda": 0.5,  # 分类损失与距离损失的平衡系数
        "optimizer": "adam",  # 优化器选择
        "lr": 0.01,  # 学习率
        "max_iter": 2000,  # 最大迭代次数
        "target_class": [0, 1],  # 目标类别（二分类）
        "binary_cat_features": True,  # 无分类特征设为True
        "vae_params": {  # VAE参数配置
            "layers": [num_features, 20, 10],  # 编码器层结构
            "train": True,  # 训练新VAE
    ...
    revise = Revise(
        mlmodel=carla_model,
        data=data_catalog,  # 需要传入数据对象
        hyperparams=revise_params
    )
    print("sample.drop('class', axis=1)*****", sample.drop('class', axis=1).shape)
    cf = revise.get_counterfactuals(factuals=sample.drop('class', axis=1))
    ...
    distance = np.linalg.norm(cf_original_scale.values[0] - centroid.values.reshape(1, -1)[0])
    ...
    test_pred = clf.decision_function(cf_original_scale.values[0].reshape(1, -1))
    ...
    for _, cf_row in cf_original_scale.iterrows():
        ...
        sparsity_count = np.sum(diff > 0.001)  # 避免浮点精度误差
...
print("test_pred_list: ", np.mean(test_pred_list))
